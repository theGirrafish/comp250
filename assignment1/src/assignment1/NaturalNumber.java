...
public class NaturalNumber {
    ...
    public NaturalNumber plus(NaturalNumber second) throws Exception {
        ...
        for (int i = 0; i < firstClone.coefficients.size(); i++) {
            int total = firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry;

            // If the sum of both digits is bigger than the base, we need a carry
            ...
            sum.coefficients.addLast(total);
        }

        // If there is a carry left over after adding all digits, we add a new digit at the head
    ...
    public NaturalNumber times(NaturalNumber multiplicand) throws Exception {
        ...
        // Multiply every digit of the multiplier by the multiplicand add its sum to the total

        for (int i = 0; i < this.coefficients.size(); i++) {
            product = product.plus(multiplicand.timesSingleDigit(this.coefficients.get(i), i));
        }
    ...
    private NaturalNumber timesSingleDigit(int digit, int position) {
        NaturalNumber lineProduct = new NaturalNumber(this.base);

        int carry = 0;

        // Multiply the digits one by one and add the product to the total

        for (int i = 0; i < this.coefficients.size(); i++) {
            int total = this.coefficients.get(i) * digit + carry;

            if (total >= this.base) {
                carry = total / this.base;
                total = total % this.base;
            } else {
            	carry = 0;
            }

            lineProduct.coefficients.addLast(total);
        }

        // If there is a carry left over after adding all digits, we add a new digit at the head

        if (carry > 0) {
            lineProduct.coefficients.addLast(carry);
        }

        // Depending on the position of the digit, pad the total with 0s

        lineProduct.timesBaseToThePower(position);
    ...
    public NaturalNumber minus(NaturalNumber second) throws Exception {
        ...
        // But the grade school algorithm sometimes requires us to "borrow" from a higher
        ...
        int borrow = 0;         // Borrow from higher digit
        int borrowed = 0;       // Digit has been borrowed from
        int digitFirst;
        int digitSecond;

        // Subtract secondDigit from firstDigit in every loop
        // After the last digit of second, set secondDigit to 0

        for (int i = 0; i < first.coefficients.size(); i++) {
            digitFirst = first.coefficients.get(i);

            if (i < second.coefficients.size()) {
                digitSecond = second.coefficients.get(i);
            } else {
                digitSecond = 0;
            }

            if (digitFirst - borrowed < digitSecond) {
                borrow = 1;
            } else {
                borrow = 0;
            }

            difference.coefficients.addLast((this.base * borrow) + digitFirst - digitSecond - borrowed);
            borrowed = borrow;
        }
        ...
        // check, which would mess up grading since correct answers would appear incorrect.
    ...
    public NaturalNumber divide(NaturalNumber divisor) throws Exception {
        ...
        int numSize = remainder.coefficients.size();
        NaturalNumber digitsToDivide = new NaturalNumber(this.base);

        // One by one, add a digit to digitsToDivide

        if (remainder.compareTo(divisor) > -1) {
            for (int i = numSize - 1; i >= 0; i--) {
                digitsToDivide.coefficients.addFirst(this.coefficients.get(i));

                // If digitsToDivide is bigger than the divisor, subtract by the highest multiple of divisor and add the mulitple
                // Otherwise, if digitsToDivide has already been divided, add 0

                if (digitsToDivide.compareTo(divisor) > -1) {
                    int fitTimes = 0;
                    NaturalNumber total = new NaturalNumber(0, this.base);

                    while (total.plus(divisor).compareTo(digitsToDivide) < 1) {
                        total = total.plus(divisor);
                        fitTimes++;
                    }

                    digitsToDivide = digitsToDivide.minus(total);
                    quotient.coefficients.addFirst(fitTimes);
                } else if (quotient.coefficients.size() != 0) {
                    quotient.coefficients.addFirst(0);
                }

                // If digitsToDivide minus the highest multiple of divisor gives exactly 0
                // Remove the 0 since it will create unwanted padding at the head

                if (digitsToDivide.coefficients.getLast() == 0) {
                	digitsToDivide.coefficients.remove();
                }
            }
        } else {
            quotient.coefficients.addFirst(0);
    ...
    // It returns -1 if a < b,  it returns 0 if a == b, and it returns 1 if a > b.
