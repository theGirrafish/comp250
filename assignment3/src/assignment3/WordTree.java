...
public class WordTree {
    ...
    // Then, add TreeNode(s) such that the word is inserted according to the specification in PDF.

    public void insert(String word) {
        //  ADD YOUR CODE BELOW HERE

        WordTreeNode lastNode = getPrefixNode(word);

        for (int i = lastNode.depth; i < word.length(); i++) {
            char charOfWord = word.charAt(i);
            lastNode = lastNode.createChild(charOfWord);
        }

        lastNode.setEndOfWord(true);
    ...
    // Given an input word, return the TreeNode corresponding to the longest prefix that is found.
    ...
    WordTreeNode getPrefixNode(String word) {
        ...
        WordTreeNode finalNode = root;

        for (int i = 0; i < word.length(); i++) {
            if (finalNode.getChild(word.charAt(i)) != null)
                finalNode = finalNode.getChild(word.charAt(i));
            else
                return finalNode;
        }
    ...
    public boolean contains(String word) {
        ...
        // return (getPrefix(word).equals(word) && getPrefixNode(word).isEndOfWord());
        return (getPrefix(word).length() == word.length() && getPrefixNode(word).isEndOfWord());
    ...
    // Helper method
    // Tree traversal in using recursion

    public void treeTraversal(ArrayList<String> matches, WordTreeNode node) {

        for (int i = 0; i < WordTreeNode.NUMCHILDREN; i++) {
            WordTreeNode nextNode = node.getChild((char)i);

            if (nextNode != null) {
                if (nextNode.isEndOfWord())
                    matches.add(nextNode.toString());

                treeTraversal(matches, nextNode);
            }
        }
    ...
    public class WordTreeNode {
        ...
        // To access it, write "WordTreeNode.NUMCHILDREN"
        ...
        char charInParent;      // Character associated with the tree edge from this node's parent to this node.
        WordTreeNode parent;
